...
//
//  Problem: myDistPred was set via clip_value(myRegOutput[0], myDistClipMin, myDistClipMax)
//  in myForwardPass, which ran the training clip AT INFERENCE TOO. The network
//  never saw gradient pressure to push above 1.0, so it saturated near 1.0.
//
//  Fix A: myDistPred is now set to myRegOutput[0] RAW — no clip at all.
//         NaN/Inf guard added separately via isnan/isinf check.
//
//  Fix B: myDistClipMin/myDistClipMax are now used ONLY in myBackwardOutput
//         to clip the gradient, not the forward value.
//         This stops the loss from punishing outputs > myDistClipMax during training,
//         freeing the network to extrapolate beyond 1.0 at inference.
//
//  Fix C: myDistClipMax raised from 1.5 to 2.0 so even the gradient clip
//         gives the network more room to push toward higher values near the
//         training boundary, improving the 9.7-but-never-10 behaviour.
//
// ======================================================
...
// Gradient clip range — applied ONLY in myBackwardOutput, NOT in myForwardPass.
// Controls the MSE gradient magnitude during training to prevent explosion.
// Upper bound > 1.0 gives the network gradient pressure to push above 1.0,
// enabling extrapolation beyond myDistMax at inference.
// Lower bound slightly negative to avoid dead-zone at zero.
// NOTE: these do NOT cap the forward prediction — myDistPred is always raw.
const float myDistClipMin = -0.1f;
const float myDistClipMax =  2.0f;
...
//        myRegOutput[0] (raw distance, linear — no clip),
//        myRegOutput[1] (raw confidence, pre-sigmoid),
//        myDistPred     (= myRegOutput[0] raw, NaN/Inf guard only — CAN exceed 1.0),
...
void myForwardPass(float* input) {
  ...
  // Apply activations
  // Distance: LINEAR — no clip here. myDistPred is the raw dot-product output.
  // This is what enables extrapolation beyond 1.0 at inference.
  // NaN/Inf guard only — if the network diverges, clamp to 0 to avoid crash.
  myDistPred = (isnan(myRegOutput[0]) || isinf(myRegOutput[0])) ? 0.0f : myRegOutput[0];
...
void myBackwardOutput(float distTarget, float confTarget) {
  ...
  // Clip the GRADIENT (not the prediction) to prevent explosion.
  // myDistClipMin/Max now only live here, not in myForwardPass.
  float myDistGrad = clip_value(2.0f * (myDistPred - distTarget), myDistClipMin * 10.0f, myDistClipMax * 10.0f);
